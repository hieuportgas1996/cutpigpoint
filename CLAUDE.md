...
- **Controls UI**: nút "Đầu hàng" + "Vote chia lại" gộp vào 1 dropdown "⋯ Tùy chọn" (`tlmn-options` trong `RoomPlayPage`) để tối giản; chỉ render khi `canSurrender || canStartVoteReset`. Đầu hàng confirm qua modal trong game (không dùng `confirm()` browser).
