...
- **Match state in-memory** trong `MatchManager` (Singleton) + `MatchTimerService` (HostedService) auto-pass 60s/lượt + auto-next-round 20s (host có thể bấm "Bắt đầu ngay" skip countdown) + auto-resolve `WhiteWinChoice` 20s + auto-finalize `PendingTrickCut` 5s. Railway restart giữa ván = mất ván — chưa persist DB.
...
- **Scoring**:
  ...
  - **Phán xử**: Nhất về mà có người chưa ra bài → mỗi victim -(4+held); Nhất ăn tổng. Case A (0 pardoned) / B (1 pardoned, -1) kết thúc ngay; Case C (≥2 pardoned) tiếp tục chơi xác định Nhì/Ba với base rank + chop-pig + đui 3♠ giữa các pardoned (pardoned chót còn 3♠ → -3 / mỗi pardoned khác +1). Held = heo + tứ quý 4 + 3-đôi 3 + 4-đôi 5. Thay thế toàn bộ scoring khi kích hoạt; ngoại lệ: bonus 3♠ (Nhất về bằng `3♠`) vẫn cộng thêm lên trên.
