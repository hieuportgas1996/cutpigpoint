...
interface UseRoomConnectionResult {
  ...
  startBreakGameNow: () => Promise<void>;
...
export function useRoomConnection(code: string | undefined): UseRoomConnectionResult {
  ...
  const activateStarOfHope = useCallback(async () => {
    const conn = connectionRef.current;
    if (!conn || conn.state !== HubConnectionState.Connected) throw new Error('Chưa kết nối phòng.');
    await conn.invoke('ActivateStarOfHope');
  }, []);

  const activateXiDach = useCallback(async () => {
    const conn = connectionRef.current;
    if (!conn || conn.state !== HubConnectionState.Connected) throw new Error('Chưa kết nối phòng.');
    await conn.invoke('ActivateXiDach');
  }, []);

  const respondGamble = useCallback(async (accept: boolean) => {
    const conn = connectionRef.current;
    if (!conn || conn.state !== HubConnectionState.Connected) throw new Error('Chưa kết nối phòng.');
  ...
  const startBreakGameNow = useCallback(async () => {
    const conn = connectionRef.current;
    if (!conn || conn.state !== HubConnectionState.Connected) throw new Error('Chưa kết nối phòng.');
    await conn.invoke('StartBreakGameNow');
  }, []);
  ...
  const submitMemoryAnswer = useCallback(async (optionIndex: number) => {
    const conn = connectionRef.current;
    if (!conn || conn.state !== HubConnectionState.Connected) throw new Error('Chưa kết nối phòng.');
    await conn.invoke('SubmitMemoryAnswer', optionIndex);
  }, []);

  const submitReflexCell = useCallback(async (cellIndex: number) => {
    const conn = connectionRef.current;
    if (!conn || conn.state !== HubConnectionState.Connected) throw new Error('Chưa kết nối phòng.');
    await conn.invoke('SubmitReflexCell', cellIndex);
  }, []);

  const submitSudokuCell = useCallback(async (cellIndex: number, value: number) => {
    const conn = connectionRef.current;
    if (!conn || conn.state !== HubConnectionState.Connected) throw new Error('Chưa kết nối phòng.');
  ...
  const drawXiDachCard = useCallback(async () => {
    const conn = connectionRef.current;
    if (!conn || conn.state !== HubConnectionState.Connected) throw new Error('Chưa kết nối phòng.');
    await conn.invoke('DrawXiDachCard');
  }, []);

  const standXiDach = useCallback(async () => {
    const conn = connectionRef.current;
    if (!conn || conn.state !== HubConnectionState.Connected) throw new Error('Chưa kết nối phòng.');
    await conn.invoke('StandXiDach');
  }, []);

  const compareXiDach = useCallback(async (targetUserId: string) => {
    const conn = connectionRef.current;
    if (!conn || conn.state !== HubConnectionState.Connected) throw new Error('Chưa kết nối phòng.');
