...
- **3♠ cuối**: `MatchPlayer.FinishedWithThreeOfSpades` set khi lá cuối đánh ra chứa `3♠` (trong `Play`/`CutNewTrick`/auto-pass). `MatchPlayer.StuckWithThreeOfSpades` set khi round end mà player còn **duy nhất 1 lá** trong tay là `3♠` (đui đúng nghĩa, không chỉ "có 3♠ giữa nhiều lá khác"). `ComputeRoundScores` áp: Nhất + cờ Finished → +(n-1) / others -1 (zero-sum); Chót + cờ Stuck → -3 / others +1 (không zero-sum khi <4 người). Cả 2 không áp khi white-win.
